...
def login_ptc(username, password):
    ...
    access_token = re.sub('&expires.*', '', r2.content.decode())
...
def main():
    ...
    #set_location_coords(args.latitude, args.longitude)
    ...
    conn = pymysql.connect(host='localhost',  user='root', passwd='root', db='pokemongo')
    ...
    #while True:
        ...
        for poke in visible:
            ...
            now = datetime.now()
            #buf = "INSERT IGNORE INTO pokemonInfo (pokemonName, latitude, longitude, hour, minute, createdDate, spawnDuration) VALUES(\'%s\',%s,%s,%s,%s,\'%s\', %s)" % ( pokemons[poke.pokemon.PokemonId - 1]['Name'], poke.Latitude, poke.Longitude, now.hour, now.minute, now, int(poke.TimeTillHiddenMs / 1000))
            buf = "CALL sp_insertIntoPokemonInfo(\'%s\',%s,%s,%s,%s,\'%s\',\'%s\', %s)" % ( pokemons[poke.pokemon.PokemonId - 1]['Name'], poke.Latitude, poke.Longitude, now.hour, now.minute, now, now, int(poke.TimeTillHiddenMs / 1000))
            #file.write(buf)
            cur = conn.cursor()
            cur.execute(buf)
            conn.commit()
        ...
        #if input('The next cell is located at %s. Keep scanning? [Y/n]' % next) in {'n', 'N'}:
        #    break       
